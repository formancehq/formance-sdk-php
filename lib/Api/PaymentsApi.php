...
/**
     ...
     *
     * @param  string $connector The connector code (required)
     ...
     * @throws \Formance\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \Formance\Model\ConnectorTask
     */
     ...
     * @throws \Formance\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \Formance\Model\ConnectorTask, HTTP status code, HTTP response headers (array of strings)
     */
    public function getConnectorTaskWithHttpInfo($connector, $task_id, string $contentType = self::contentTypes['getConnectorTask'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Formance\Model\ConnectorTask' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\Formance\Model\ConnectorTask' !== 'string') {
                            $content = json_decode($content);
                        }
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\Formance\Model\ConnectorTask', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\Formance\Model\ConnectorTask';
            if ($returnType === '\SplFileObject') {
                $content = $response->getBody(); //stream goes to serializer
            } else {
                $content = (string) $response->getBody();
