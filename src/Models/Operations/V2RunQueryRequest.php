<?php

/**
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

declare(strict_types=1);

namespace formance\stack\Models\Operations;

use formance\stack\Utils\SpeakeasyMetadata;
class V2RunQueryRequest
{
    /**
     *
     * @var V2RunQueryRequestBody $requestBody
     */
    #[SpeakeasyMetadata('request:mediaType=application/json')]
    public V2RunQueryRequestBody $requestBody;

    /**
     * Query template ID.
     *
     * @var string $id
     */
    #[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=id')]
    public string $id;

    /**
     * Name of the ledger.
     *
     * @var string $ledger
     */
    #[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=ledger')]
    public string $ledger;

    /**
     * Schema version to use for validation
     *
     * @var string $schemaVersion
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=schemaVersion')]
    public string $schemaVersion;

    /**
     * Parameter used in pagination requests. Maximum page size is set to 15.
     *
     * Set to the value of next for the next page of results.
     * Set to the value of previous for the previous page of results.
     * No other parameters can be set when this parameter is set.
     *
     *
     * @var ?string $cursor
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=cursor')]
    public ?string $cursor = null;

    /**
     *
     * @var ?string $expand
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=expand')]
    public ?string $expand = null;

    /**
     * Deprecated: Use sort param
     *
     * @var ?V2RunQueryQueryParamOrder $order
     * @deprecated  field: This will be removed in a future release, please migrate away from it as soon as possible.
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=order')]
    public ?V2RunQueryQueryParamOrder $order = null;

    /**
     * The maximum number of results to return per page.
     *
     *
     *
     * @var ?int $pageSize
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=pageSize')]
    public ?int $pageSize = null;

    /**
     *
     * @var ?\DateTime $pit
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=pit,dateTimeFormat=Y-m-d\TH:i:s.up')]
    public ?\DateTime $pit = null;

    /**
     *
     * @var ?bool $reverse
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=reverse')]
    public ?bool $reverse = null;

    /**
     * Sort results using a field name and order (ascending or descending).
     *
     * Format: `<field>:<order>`, where `<field>` is the field name and `<order>` is either `asc` or `desc`.
     *
     *
     * @var ?string $sort
     */
    #[SpeakeasyMetadata('queryParam:style=form,explode=true,name=sort')]
    public ?string $sort = null;

    /**
     * @param  V2RunQueryRequestBody  $requestBody
     * @param  string  $id
     * @param  string  $ledger
     * @param  string  $schemaVersion
     * @param  ?string  $cursor
     * @param  ?string  $expand
     * @param  ?V2RunQueryQueryParamOrder  $order
     * @param  ?int  $pageSize
     * @param  ?\DateTime  $pit
     * @param  ?bool  $reverse
     * @param  ?string  $sort
     * @phpstan-pure
     */
    public function __construct(V2RunQueryRequestBody $requestBody, string $id, string $ledger, string $schemaVersion, ?string $cursor = null, ?string $expand = null, ?V2RunQueryQueryParamOrder $order = null, ?int $pageSize = null, ?\DateTime $pit = null, ?bool $reverse = null, ?string $sort = null)
    {
        $this->requestBody = $requestBody;
        $this->id = $id;
        $this->ledger = $ledger;
        $this->schemaVersion = $schemaVersion;
        $this->cursor = $cursor;
        $this->expand = $expand;
        $this->order = $order;
        $this->pageSize = $pageSize;
        $this->pit = $pit;
        $this->reverse = $reverse;
        $this->sort = $sort;
    }
}